...
export default class ThreejsSceneLayer implements CustomLayerInterface {
  ...
  render = (gl: WebGL2RenderingContext, matrix: number[]) => {
    if (!this._map ||!this._renderer ||!this._scene ||!this._cameraForRender) {
      return;
    }
  ...
  addTileset(tilesetOptions: TilesetOptions): Tileset {
    if (!this._renderer ||!this._sceneRoot ||!this._refCenter) {
      throw new Error("ThreejsSceneLayer is not initialized");
    }

    this.removeTileset(tilesetOptions.id);

    const newTileset: Tileset = new Tileset(tilesetOptions);
    newTileset.addToScene(this._sceneRoot, this._refCenter, this._renderer);
  ...
  removeTileset(tilesetId: string) {
    ...
    if (tileset) {
      tileset.removeFromScene(this._sceneRoot);
